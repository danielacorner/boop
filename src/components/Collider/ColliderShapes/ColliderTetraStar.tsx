...
const createTetrahedronShape = (radius: number, rotation: [number, number, number] = [0, 0, 0]) => {
  ...
  // Convert to convex props for physics
...
export function ColliderTetraStar({ geometryType = "tetrahedron_star" }: { geometryType?: GeometryType }) {
  ...
  const tetraUpShape = useMemo(() => 
    createTetrahedronShape(colliderRadius * 1.4, [Math.PI/5, Math.PI/4, 0]), 
    [colliderRadius]
  );
  
  const tetraDownShape = useMemo(() => 
    createTetrahedronShape(colliderRadius * 1.4, [Math.PI/5 + Math.PI/2, Math.PI/4 + Math.PI/2, Math.PI/2]), 
    [colliderRadius]
  );
  ...
  return (
    <animated.group name="tetraStarGroup" ref={starRef as any} scale={scale}>
      {/* Visual representation of tetrahedron star */}
